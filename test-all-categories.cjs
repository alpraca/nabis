...
const testSubcategoryEndpoints = [
  ...
  '/api/products?category=dermokozmetike&subcategory=Floket',
  '/api/products?category=higjena&subcategory=Goja',
  '/api/products?category=farmaci&subcategory=mireqenia-seksuale',
  '/api/products?category=mama-dhe-bebat&subcategory=Suplementa'
]

console.log('🌐 TESTING API ENDPOINTS (port 3001):\n')
...
const makeRequest = (endpoint) => {
  return new Promise((resolve) => {
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          // API returns { products: [...] } not direct array
          ...
          console.log(`${status} ${endpoint.type.padEnd(15)} ${displayUrl.padEnd(40)} → ${count.toString().padStart(4)} products`)
          
          completed++
          resolve({ success: true, count })
        } catch (e) {
          console.log(`❌ ${endpoint.type.padEnd(15)} ${endpoint.url.substring(0, 40).padEnd(40)} → ERROR: ${e.message}`)
          completed++
    ...
    req.on('error', (e) => {
      console.log(`❌ ${endpoint.type.padEnd(15)} ${endpoint.url.substring(0, 40).padEnd(40)} → CONNECTION ERROR`)
      completed++
      resolve({ success: false, count: 0 })
    })
