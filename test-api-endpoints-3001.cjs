...
const makeRequest = (endpoint) => {
  return new Promise((resolve) => {
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          console.log(`${status} ${endpoint.type.padEnd(15)} ${displayUrl.padEnd(45)} → ${count.toString().padStart(4)} products`)
          
          completed++
          resolve({ success: true, count })
        } catch (e) {
          console.log(`❌ ${endpoint.type.padEnd(15)} ${endpoint.url.padEnd(45)} → ERROR: ${e.message}`)
          completed++
    ...
    req.on('error', (e) => {
      console.log(`❌ ${endpoint.type.padEnd(15)} ${endpoint.url.padEnd(45)} → CONNECTION ERROR`)
      completed++
      resolve({ success: false, count: 0 })
    })
